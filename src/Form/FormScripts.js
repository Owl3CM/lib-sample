...
const _extract = (group, defaultQP, includeNone) => {
    group?.children.map((query) => {
        ...
        if (query.type === "selector") {
            query.className = query.options.find((o) => o.id == query.value)?.className || query.className;
        ...
        if (includeNone || Utils.hasValue(query.value)) defaultQP[query.key] = { value: query.value, title: query.title || "_" }; //, clear: query.showInClearBar && query.value !== false, type: query.type };
    });
};
